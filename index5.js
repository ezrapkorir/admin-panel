...
import { getAuth, onAuthStateChanged } from "https://www.gstatic.com/firebasejs/10.14.0/firebase-auth.js"; // Import Firebase Authentication
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        // User is signed in
        loadData(); // Load user data if logged in
    } else {
...
function displayUsersData(usersData) {
    ...
    usersData.forEach((user) => {
        ...
        // Update status to 'approved' when Activate button is clicked
...
function loadData() {
    onValue(usersRef, (snapshot) => {
        ...
        // Filter users whose status is 'suspended' or 'deactivated' and roles of interest
